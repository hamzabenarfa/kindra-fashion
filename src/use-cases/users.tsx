...
import { FacebookUser } from "@/app/api/login/facebook/callback/route";
...
export async function createFacebookUserUseCase(facebookUser: FacebookUser) {
  let existingUser = await getUserByEmail(facebookUser.email);

  if (!existingUser) {
    existingUser = await createUser(facebookUser.email);
  }

  await createAccountViaFacebook(existingUser.id, facebookUser.id);

  await createProfile(existingUser.id, facebookUser.name, facebookUser.picture?.data?.url);
...
// Notifications removed as they were tied to groups/posts
